...
public class LiveTest extends AbstractTestCase {
	...
	public void testSavannah() throws Exception {
		...
		grab(
				"http://cvs.savannah.gnu.org/viewcvs/classpath/classpath/faq/?root=Web",
				"ViewCvs0_9");
	...
	public void testHelixCommunity() throws Exception {
		...
		grab(
				"https://helixcommunity.org/plugins/scmcvs/cvsweb.php/www/?cvsroot=template",
				"ViewCvs1_0");
		assertContainsFile("index.html");
	}
	...
	private void grab(String url) {
		CVSGrab.main(new String[] { "-url", url, "-packageDir", "test",
				"-destDir", tmpDir.getName() });
	}

	private void grab(String url, String webInterface) {
		CVSGrab.main(new String[] { "-url", url, "-packageDir", "test",
				"-destDir", tmpDir.getName(), "-webInterface", webInterface });
	}
